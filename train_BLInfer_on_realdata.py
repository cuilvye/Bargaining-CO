...
This is a python file for training BLInfer model with real bargaining dataset.
@author: Lvye Cui
...
seller_set = np.unique(trainPrior_dataframe['anon_slr_id']).tolist()
seller_num = len(seller_set) 
print('seller number: {}'.format(seller_num), end = '\n')  

path_tuple = (path_res, path_model)
start_time = time.time()
learning_model_paras_V0(trainPrior_dataframe, validPrior_dataframe, t_set, epsilon, classes, epochs, batch_size, lr, syn_tag, vs_disType, dis_pars, (0, 0), path_tuple)
